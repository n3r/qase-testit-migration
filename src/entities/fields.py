...
class Fields:
    ...
    async def import_fields_async(self):
        ...
        self.logger.log('[Fields] Loading custom fields from TestIT')
        testit_custom_fields = await self.pools.tr(self.testit.get_case_fields)
        ...
        total = len(testit_custom_fields)
        
        self.logger.log(f'[Fields] Found {str(total)} custom fields')

        fields_to_import = self._get_fields_to_import(testit_custom_fields)

        i = 0
        self.logger.print_status('Importing custom fields', i, total)
        self.mappings.stats.add_custom_field('testit', total)
        async with asyncio.TaskGroup() as tg:
            for field in testit_custom_fields:
                i += 1
    ...
    async def _create_types_map(self):
        ...
        tr_types = await self.pools.tr(self.testit.get_case_types)
        qase_types = []
    ...
    async def _create_priorities_map(self):
        ...
        tr_priorities = await self.pools.tr(self.testit.get_priorities)
        qase_priorities = []
    ...
    async def _create_result_statuses_map(self):
        ...
        tr_statuses = await self.pools.tr(self.testit.get_result_statuses)
        qase_statuses = []
    ...
    def _create_case_statuses_map(self):
        ...
        tr_statuses = self.testit.get_case_statuses()
        qase_statuses = []
